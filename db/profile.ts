...
export async function updateProfile(userId: string, input: { nickname: string; avatar: string; frame: string; gameRank: string }) {
  ...
  const [updated] = await db.update(profiles).set({ nickname, avatar: input.avatar.slice(0, 40), frame: input.frame.slice(0, 40), gameRank: input.gameRank.slice(0, 40), updatedAt: new Date().toISOString() }).where(eq(profiles.userId, userId)).returning();
