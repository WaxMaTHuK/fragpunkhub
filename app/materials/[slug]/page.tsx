import { ArrowLeft, ArrowUpRight, ShieldCheck } from "lucide-react";
import { notFound } from "next/navigation";
import { listPosts } from "@/db/content";
import { DEFAULT_POSTS, TYPE_LABELS, type HubPost } from "@/lib/hub-content";
import { uploadedImageUrl, youtubeEmbedUrl } from "@/lib/video";
import { parseWeaponContent, WEAPON_FIRE_MODE_LABELS, type WeaponFields } from "@/lib/weapon-content";
import { MaterialProgress } from "./material-progress";

export const dynamic = "force-dynamic";
async function getPost(slug: string): Promise<HubPost | undefined> { try { return (await listPosts(false)).find((post) => post.slug === slug); } catch { return DEFAULT_POSTS.map((post) => ({ ...post, updatedAt: new Date(0).toISOString() })).find((post) => post.slug === slug); } }

export default async function MaterialPage({ params }: { params: Promise<{ slug: string }> }) {
  const post = await getPost((await params).slug); if (!post) notFound();
  const weapon = post.type === "weapon" ? parseWeaponContent(post.content) : null; const content = weapon ? weapon.description : post.content; const weaponImage = post.slug === "мясник-1fe0ee" ? "https://raw.githubusercontent.com/WaxMaTHuK/fragpunkhub/main/public/images/weapons/myasnik.jpg" : weapon?.image;
  return <div className="site-shell"><header className="site-header"><div className="wrap header-row"><a className="brand" href="/" aria-label="FragPunk Hub — на главную"><span className="brand-mark" aria-hidden="true" /><span className="brand-text"><strong>FRAGPUNK</strong><small>HUB.RU</small></span></a><nav className="main-nav" aria-label="Основная навигация"><a href="/#materials">Материалы</a><a href="/#sections">Разделы</a><a className="admin-link" href="/admin"><ShieldCheck size={17} /> Редактор</a></nav></div></header><main className="wrap material-page"><a className="back-link" href="/#materials"><ArrowLeft size={16} /> Все материалы</a><MaterialProgress postId={post.id} /><article className={`material-article accent-${post.accent}`}>
    {weapon && <div className="weapon-overview">{weaponImage ? <figure className="article-image weapon-hero-image"><img src={weaponImage} alt={post.title} /></figure> : <div className="weapon-hero-placeholder">Изображение оружия</div>}<WeaponStats weapon={weapon} /></div>}
    <div className="material-meta"><span>{TYPE_LABELS[post.type]}</span><i>·</i><span>{post.readTime}</span></div><h1>{post.title}</h1><p className="material-summary">{post.summary}</p>{weapon && weapon.lancers.length > 0 && <div className="lancer-match"><strong>Подходит лансерам</strong><div className="lancer-avatars">{weapon.lancers.map((image) => <img src={image} alt="Подходящий лансер" key={image} />)}</div></div>}
    <div className="article-body">{content.split("\n").filter(Boolean).map((paragraph, index) => { const line = paragraph.trim(); const videoUrl = youtubeEmbedUrl(line); const imageUrl = uploadedImageUrl(line); return videoUrl ? <div className="video-embed" key={index}><iframe src={videoUrl} title={`Видео: ${post.title}`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div> : imageUrl ? <figure className="article-image" key={index}><img src={imageUrl} alt={`Иллюстрация: ${post.title}`} /></figure> : <p key={index}>{paragraph}</p>; })}{weapon?.video && youtubeEmbedUrl(weapon.video) && <div className="video-embed"><iframe src={youtubeEmbedUrl(weapon.video)!} title={`Видео: ${post.title}`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div>}</div><a className="material-back" href="/#materials">К материалам <ArrowUpRight size={16} /></a>
  </article></main><footer><div className="wrap footer-row"><p>Неофициальный русскоязычный информационный портал. FragPunk и связанные материалы принадлежат их правообладателям.</p><strong>Проект <span>WaxMaTHuK</span></strong></div></footer></div>;
}

function WeaponStats({ weapon }: { weapon: WeaponFields }) {
  const firearmDamage = [{ label: "Голова", values: weapon.damageHead }, { label: "Корпус", values: weapon.damageBody }, { label: "Конечности", values: weapon.damageLimbs }].filter((row) => row.values.some(Boolean));
  const meleeDamage = [{ label: "Лёгкая атака", values: [weapon.meleeLightDamage, weapon.meleeLightBackstab] }, { label: "Тяжёлая атака", values: [weapon.meleeHeavyDamage, weapon.meleeHeavyBackstab] }, { label: "Заряженная атака", values: [weapon.meleeChargeDamage, weapon.meleeChargeBackstab] }].filter((row) => row.values.some(Boolean));
  const details = weapon.kind === "melee" ? [["Дальность рывка", weapon.dashDistance], ["Время зарядки", weapon.chargeTime], ["Область поражения", weapon.attackArea], ["Приседание", weapon.crouchSpeed], ["Ходьба", weapon.walkSpeed], ["Бег", weapon.runSpeed]].filter((item) => item[1]) : [["Стрельба", WEAPON_FIRE_MODE_LABELS[weapon.fireMode]], ["Магазин", [weapon.magazine, weapon.reserveAmmo].filter(Boolean).join(" / ")], ["Приседание", weapon.crouchSpeed], ["Ходьба", weapon.walkSpeed], ["Бег", weapon.runSpeed], ["Скорострельность", weapon.fireRate], ["Приближение", weapon.zoom], ["Вход в прицел", weapon.adsTime]].filter((item) => item[1]);
  const damage = weapon.kind === "melee" ? meleeDamage : firearmDamage;
  const thirdRange = weapon.damageRanges[2]?.trim() ?? "";
  const hasThirdFirearmRange = firearmDamage.some((row) => Boolean(row.values[2])) || (Boolean(thirdRange) && !/^0\s*[–-]\s*0\s*м?$/i.test(thirdRange));
  const columnCount = weapon.kind === "melee" ? 2 : hasThirdFirearmRange ? 3 : 2;
  const headers = weapon.kind === "melee" ? ["Обычный", "Удар в спину"] : weapon.damageRanges.slice(0, columnCount);
  const tableClass = `damage-table ${weapon.kind === "melee" ? "melee-damage-table" : ""} damage-columns-${columnCount}`;
  return <aside className="weapon-stats"><h2>Характеристики</h2>{damage.length > 0 && <div className={tableClass}><div className="damage-head"><span>{weapon.kind === "melee" ? "Атака" : "Урон"}</span>{headers.map((header, index) => <b key={`${header}-${index}`}>{header}</b>)}</div>{damage.map((row) => <div className="damage-row" key={row.label}><strong>{row.label}</strong>{row.values.slice(0, columnCount).map((value, index) => <span key={index}>{value}</span>)}</div>)}</div>}{details.length > 0 && <div className="weapon-stat-grid">{details.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div>}</aside>;
}
