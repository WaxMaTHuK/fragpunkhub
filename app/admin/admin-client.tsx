...
function ShardCardEditor({ fields, onChange }: { fields: ShardFields; onChange: (changes: Partial<ShardFields>) => void }) {
  ...
  return <div className="field-wide shard-editor"><div className="shard-editor-title"><div><p className="eyebrow">Расширенный редактор</p><h2>Фрагмент карта</h2></div><span>Все поля появятся в каталоге автоматически</span></div><div className="shard-editor-grid"><div className="field field-wide"><Label htmlFor="shard-image"><Image size={16} /> Изображение карточки</Label><Input id="shard-image" value={fields.image} onChange={(event) => onChange({ image: event.target.value })} placeholder="https://raw.githubusercontent.com/.../card.png" /><p className="field-help">Вставь прямую ссылку на изображение из папки public/images.</p></div><div className="field field-wide"><Label htmlFor="shard-description">Полное описание эффекта</Label><Textarea id="shard-description" value={fields.description} onChange={(event) => onChange({ description: event.target.value })} placeholder="Что делает карточка, когда её лучше выбирать и как использовать…" rows={8} /></div><div className="field field-wide"><Label htmlFor="shard-video"><Video size={17} /> Видео с примером</Label><Input id="shard-video" value={fields.video} onChange={(event) => onChange({ video: event.target.value })} placeholder="https://www.youtube.com/watch?v=..." /></div><div className="field field-wide lancer-editor"><div className="field-label-row"><div><Label>Подходящие лансеры</Label><p className="field-help">Добавь прямые ссылки на их маленькие аватарки.</p></div><Button type="button" variant="outline" size="sm" onClick={addLancer}><Plus /> Добавить лансера</Button></div>{fields.lancers.length ? <div className="lancer-editor-list">{fields.lancers.map((lancer, index) => <div className="lancer-editor-row" key={index}>{lancer && imagePattern.test(lancer) ? <img src={lancer} alt="" /> : <span className="lancer-placeholder">{index + 1}</span>}<Input value={lancer} onChange={(event) => changeLancer(index, event.target.value)} placeholder="Ссылка на аватарку лансера" /><Button type="button" variant="ghost" size="icon" onClick={() => removeLancer(index)} aria-label="Удалить лансера"><Trash2 /></Button></div>)}</div> : <div className="lancer-editor-empty">Подходящие лансеры пока не добавлены</div>}</div></div></div>;
}

function WeaponEditor({ fields, onChange }: { fields: WeaponFields; onChange: (changes: Partial<WeaponFields>) => void }) {
  function addLancer() { const url = window.prompt("Вставь прямую ссылку на аватарку лансера:"); if (url?.trim()) onChange({ lancers: [...fields.lancers, url.trim()] }); }
  function damageRow(key: "damageHead" | "damageBody" | "damageLimbs", title: string) { return <div className="damage-editor-row"><strong>{title}</strong>{fields[key].map((value, index) => <Input key={index} value={value} onChange={(e) => { const next = [...fields[key]] as [string,string,string]; next[index] = e.target.value; onChange({ [key]: next }); }} placeholder="Урон" />)}</div>; }
  return <div className="field-wide shard-editor weapon-editor"><div className="shard-editor-title"><div><p className="eyebrow">Расширенный редактор</p><h2>Оружие</h2></div><span>Эти параметры используются в тесте подбора</span></div><div className="shard-editor-grid">
    <div className="field field-wide"><Label htmlFor="weapon-image"><Image size={16} /> Изображение оружия</Label><Input id="weapon-image" value={fields.image} onChange={(e) => onChange({ image: e.target.value })} placeholder="https://raw.githubusercontent.com/.../weapon.png" /></div>
    <div className="field field-wide"><Label htmlFor="weapon-description">Полное описание</Label><Textarea id="weapon-description" value={fields.description} onChange={(e) => onChange({ description: e.target.value })} placeholder="Характеристики, сильные стороны и советы…" rows={8} /></div>
    <div className="field field-wide"><Label htmlFor="weapon-video"><Video size={17} /> Обзор на YouTube</Label><Input id="weapon-video" value={fields.video} onChange={(e) => onChange({ video: e.target.value })} placeholder="https://www.youtube.com/watch?v=..." /></div>
    <div className="weapon-quiz-fields field-wide">
      <div className="field"><Label>Тип оружия</Label><Select value={fields.kind} onValueChange={(v) => onChange({ kind: v as WeaponFields["kind"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="firearm">Огнестрельное</SelectItem><SelectItem value="melee">Ближний бой</SelectItem></SelectContent></Select></div>
      <div className="field"><Label>Дистанция</Label><Select value={fields.range} onValueChange={(v) => onChange({ range: v as WeaponFields["range"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="close">Ближняя</SelectItem><SelectItem value="medium">Средняя</SelectItem><SelectItem value="long">Дальняя</SelectItem></SelectContent></Select></div>
      <div className="field"><Label>Стиль игры</Label><Select value={fields.style} onValueChange={(v) => onChange({ style: v as WeaponFields["style"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="aggressive">Агрессивный</SelectItem><SelectItem value="balanced">Универсальный</SelectItem><SelectItem value="careful">Осторожный</SelectItem></SelectContent></Select></div>
      <div className="field"><Label>Сложность контроля</Label><Select value={fields.control} onValueChange={(v) => onChange({ control: v as WeaponFields["control"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="easy">Лёгкая</SelectItem><SelectItem value="medium">Средняя</SelectItem><SelectItem value="hard">Высокая</SelectItem></SelectContent></Select></div>
      <div className="field"><Label>Роль</Label><Select value={fields.role} onValueChange={(v) => onChange({ role: v as WeaponFields["role"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="entry">Первый номер</SelectItem><SelectItem value="support">Поддержка</SelectItem><SelectItem value="universal">Универсал</SelectItem></SelectContent></Select></div>
      <div className="field"><Label>Режим стрельбы</Label><Select value={fields.fireMode} onValueChange={(v) => onChange({ fireMode: v as WeaponFields["fireMode"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="automatic">Автоматический</SelectItem><SelectItem value="burst">Очередями</SelectItem><SelectItem value="single">Одиночный</SelectItem><SelectItem value="bolt">Болтовый затвор</SelectItem><SelectItem value="pump">Помповый</SelectItem></SelectContent></Select></div>
    </div>
    <div className="field field-wide weapon-stats-editor"><Label>Урон по дистанциям</Label><p className="field-help">Перепиши значения из таблицы оружия в игре.</p><div className="damage-editor-head"><span>Попадание</span><strong>0–10 м</strong><strong>10–25 м</strong><strong>25–60 м</strong></div>{damageRow("damageHead", "Голова")}{damageRow("damageBody", "Корпус")}{damageRow("damageLimbs", "Конечности")}</div>
    <div className="weapon-number-grid field-wide"><div className="field"><Label>Магазин</Label><Input value={fields.magazine} onChange={(e) => onChange({ magazine: e.target.value })} placeholder="8" /></div><div className="field"><Label>Боезапас</Label><Input value={fields.reserveAmmo} onChange={(e) => onChange({ reserveAmmo: e.target.value })} placeholder="24" /></div><div className="field"><Label>Скорострельность</Label><Input value={fields.fireRate} onChange={(e) => onChange({ fireRate: e.target.value })} placeholder="100/мин" /></div><div className="field"><Label>Приближение</Label><Input value={fields.zoom} onChange={(e) => onChange({ zoom: e.target.value })} placeholder="1×" /></div><div className="field"><Label>Вход в прицел</Label><Input value={fields.adsTime} onChange={(e) => onChange({ adsTime: e.target.value })} placeholder="0.10 с" /></div><div className="field"><Label>Задержка при смене</Label><Input value={fields.equipTime} onChange={(e) => onChange({ equipTime: e.target.value })} placeholder="0.01 с" /></div><div className="field"><Label>Скорость приседа</Label><Input value={fields.crouchSpeed} onChange={(e) => onChange({ crouchSpeed: e.target.value })} placeholder="2.2 м/с" /></div><div className="field"><Label>Скорость ходьбы</Label><Input value={fields.walkSpeed} onChange={(e) => onChange({ walkSpeed: e.target.value })} placeholder="3.5 м/с" /></div><div className="field"><Label>Скорость бега</Label><Input value={fields.runSpeed} onChange={(e) => onChange({ runSpeed: e.target.value })} placeholder="4.9 м/с" /></div></div>
    {fields.kind === "melee" && <MeleeStatsEditor fields={fields} onChange={onChange} />}
    <div className="field field-wide lancer-editor"><div className="field-label-row"><div><Label>Подходящие лансеры</Label><p className="field-help">Добавь прямые ссылки на их аватарки.</p></div><Button type="button" variant="outline" size="sm" onClick={addLancer}><Plus /> Добавить лансера</Button></div>{fields.lancers.length ? <div className="lancer-editor-list">{fields.lancers.map((lancer, index) => <div className="lancer-editor-row" key={index}>{lancer && imagePattern.test(lancer) ? <img src={lancer} alt="" /> : <span className="lancer-placeholder">{index + 1}</span>}<Input value={lancer} onChange={(e) => onChange({ lancers: fields.lancers.map((item, i) => i === index ? e.target.value : item) })} placeholder="Ссылка на аватарку лансера" /><Button type="button" variant="ghost" size="icon" onClick={() => onChange({ lancers: fields.lancers.filter((_, i) => i !== index) })} aria-label="Удалить лансера"><Trash2 /></Button></div>)}</div> : <div className="lancer-editor-empty">Подходящие лансеры пока не добавлены</div>}</div>
  </div></div>;
}

function MeleeStatsEditor({ fields, onChange }: { fields: WeaponFields; onChange: (changes: Partial<WeaponFields>) => void }) {
  const rows: Array<["meleeLightDamage" | "meleeHeavyDamage" | "meleeChargeDamage", "meleeLightBackstab" | "meleeHeavyBackstab" | "meleeChargeBackstab", string]> = [["meleeLightDamage", "meleeLightBackstab", "Лёгкая атака"], ["meleeHeavyDamage", "meleeHeavyBackstab", "Тяжёлая атака"], ["meleeChargeDamage", "meleeChargeBackstab", "Заряженная атака"]];
  return <div className="field-wide weapon-stats-editor melee-stats-editor"><Label>Характеристики ближнего боя</Label><p className="field-help">Пустые поля не появятся на странице оружия.</p><div className="damage-editor-head"><span>Атака</span><strong>Обычный</strong><strong>Удар в спину</strong></div>{rows.map(([normal, backstab, title]) => <div className="damage-editor-row" key={title}><strong>{title}</strong><Input value={fields[normal]} onChange={(e) => onChange({ [normal]: e.target.value })} placeholder="Урон" /><Input value={fields[backstab]} onChange={(e) => onChange({ [backstab]: e.target.value })} placeholder="Урон" /></div>)}<div className="weapon-number-grid"><div className="field"><Label>Дальность рывка</Label><Input value={fields.dashDistance} onChange={(e) => onChange({ dashDistance: e.target.value })} placeholder="3–3.3 м" /></div><div className="field"><Label>Время зарядки</Label><Input value={fields.chargeTime} onChange={(e) => onChange({ chargeTime: e.target.value })} placeholder="1.5 с" /></div><div className="field"><Label>Область поражения</Label><Input value={fields.attackArea} onChange={(e) => onChange({ attackArea: e.target.value })} placeholder="2.1–3.7 м" /></div></div></div>;
}
