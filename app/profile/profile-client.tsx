...
export function ProfileClient({ initialProfile }: { initialProfile: Profile }) {
  ...
  const maxLevel = 100;
  const levelStartXp = (Math.min(profile.level, maxLevel) - 1) * 100;
  const xpInLevel = Math.min(100, Math.max(0, profile.xp - levelStartXp));
  const atMaxLevel = profile.level >= maxLevel;
